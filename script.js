...
const translations = {
  es: {
    ...
    documentacionBtn: "📚 Aprende más sobre el proyecto", manualBtn: "📖 Abrir manual completo (PDF)",
    cerrarBtn: "✖ CERRAR SISTEMA", clearHistorial: "🗑 LIMPIAR TODO",
  ...
  en: {
    ...
    documentacionBtn: "📚 Learn more about the project", manualBtn: "📖 Open full manual (PDF)",
    cerrarBtn: "✖ CLOSE SYSTEM", clearHistorial: "🗑 CLEAR ALL",
  ...
  ja: {
    ...
    documentacionBtn: "📚 プロジェクトについて", manualBtn: "📖 マニュアルを開く (PDF)",
    cerrarBtn: "✖ 閉じる", clearHistorial: "🗑 すべてクリア",
...
function setLanguage(lang) {
  ...
  // panelBtns[5] es cerrarSistema
  document.querySelector(".btn-limpiar").innerHTML = t.clearHistorial;
  // Actualizar etiqueta del slider de tema
  const tema = document.documentElement.getAttribute("data-theme");
  document.getElementById("toggle-label").innerText =
    tema === "dark"
      ? (lang === "es" ? "MODO CLARO" : lang === "en" ? "LIGHT MODE" : "ライトモード")
      : (lang === "es" ? "MODO OSCURO" : lang === "en" ? "DARK MODE" : "ダークモード");
}
...
function aplicarTema(tema) {
  ...
  document.getElementById("toggle-label").innerText =
    tema === "dark"
      ? (currentLang === "es" ? "MODO CLARO" : currentLang === "en" ? "LIGHT MODE" : "ライトモード")
      : (currentLang === "es" ? "MODO OSCURO" : currentLang === "en" ? "DARK MODE" : "ダークモード");
}
...
// Cargar tema guardado antes del DOM
(function () {
...
async function renderHistorial() {
  ...
  if (!builds.length) { div.innerHTML = '<p class="no-historial">Sin configuraciones guardadas.</p>'; return; }
...
async function registrarUsuario() {
  let nu = prompt("Crea un usuario");   if (!nu) return;
  let np = prompt("Crea una contraseña"); if (!np) return;
...
async function exportarPDF() {
  ...
  const mlLink         = `https://listado.mercadolibre.com.mx/${busqueda}`;
  doc.setFontSize(8);
  ...
  doc.textWithLink("Buscar en MercadoLibre México", 14, finalY + 58, { url: mlLink });
